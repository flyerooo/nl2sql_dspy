# NL2SQL-IR: 基于中间表示的问题到SQL转换

一个使用中间表示(Intermediate Representation, IR)架构的 Text-to-SQL 系统,结合 DSPy 框架实现稳健的自然语言到 SQL 查询转换。

## 📋 目录

- [项目概述](#项目概述)
- [核心架构](#核心架构)
- [快速开始](#快速开始)
- [详细使用](#详细使用)
- [项目结构](#项目结构)
- [配置说明](#配置说明)
- [测试](#测试)
- [开发指南](#开发指南)

## 项目概述

### 为什么使用 IR 架构?

直接让 LLM 生成 SQL 字符串(Direct-to-SQL)虽然简单,但存在诸多问题:
- ❌ 容易产生语法错误
- ❌ 表名/列名幻觉
- ❌ 难以调试和维护
- ❌ 存在 SQL 注入风险

**IR 架构的优势:**
- ✅ 结构化、可验证的输出
- ✅ 解耦 LLM 理解和 SQL 生成
- ✅ 支持语义层抽象(自动处理 JOIN)
- ✅ 易于调试和优化
- ✅ 更好的安全性

### 技术栈

- **DSPy**: LLM 程序框架,支持类型化预测器和自动优化
- **Pydantic**: 数据验证和类型检查
- **JSON5**: 支持注释的配置文件格式

## 核心架构

```
┌─────────────────┐
│  自然语言问题    │
│  "中国区销售额   │
│   最高的5个产品" │
└────────┬────────┘
         │
         ▼
┌─────────────────────────────────────┐
│ 步骤1: 候选实体提取                  │
│ (SimpleCandidateExtractor)          │
│ - 基于关键词匹配筛选相关实体         │
│ - 可替换为更复杂的语义检索系统       │
└────────┬────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────┐
│ 步骤2: 生成中间表示 (IR)            │
│ (ClauseDeconstructor + DSPy)        │
│ - 三阶段 LLM 调用                   │
│ - Pydantic 类型保证                 │
└────────┬────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────┐
│ IR (JSON 结构)                      │
│ {                                   │
│   "projections": [...],             │
│   "filters": {...},                 │
│   "group_by": [...],                │
│   ...                               │
│ }                                   │
└────────┬────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────┐
│ 步骤3: 编译为 SQL                   │
│ (SQLCompiler)                       │
│ - 自动计算 JOIN 路径                │
│ - 生成完整 SQL                      │
└────────┬────────────────────────────┘
         │
         ▼
┌─────────────────┐
│  SQL 查询       │
│  SELECT ...     │
│  FROM ...       │
│  WHERE ...      │
└─────────────────┘
```

## 快速开始

### 1. 安装依赖

```bash
pip install dspy-ai pydantic json5
```

### 2. 配置 LLM

编辑 `src/nl2sql_pipeline.py` 或测试脚本,配置你的 LLM:

```python
# 选项1: OpenAI
gpt4_turbo = dspy.OpenAI(
    model='gpt-4-1106-preview',
    api_key='YOUR_API_KEY',
    max_tokens=4096
)
dspy.settings.configure(lm=gpt4_turbo)

# 选项2: 本地 Ollama
ollama_lm = dspy.OllamaLocal(
    model='llama3',
    max_tokens=4096
)
dspy.settings.configure(lm=ollama_lm)
```

### 3. 运行示例

```bash
cd src
python nl2sql_pipeline.py
```

### 4. 运行测试

```bash
cd tests
python test_pipeline.py
```

## 详细使用

### 基本用法

```python
from nl2sql_pipeline import NL2SQLPipeline

# 初始化流程
pipeline = NL2SQLPipeline()

# 执行查询
result = pipeline.execute(
    question="查询中国区销售额最高的5个产品",
    return_ir=True,  # 可选:返回中间表示
    verbose=True     # 可选:打印详细日志
)

print(result["sql"])
```

### 自定义语义层

编辑 `entity_map.json5` 来定义你的业务实体:

```json5
{
  "entities": {
    "your_entity": {
      "table": "your_table",
      "column": "your_column",
      "type": "attribute"
    }
  },
  "foreign_keys": [
    {
      "from_table": "table_a",
      "from_column": "id",
      "to_table": "table_b",
      "to_column": "foreign_id"
    }
  ]
}
```

### 高级:自定义候选提取器

可以替换默认的 `SimpleCandidateExtractor` 为更复杂的实现:

```python
# 实现自定义提取器
class CustomCandidateExtractor:
    def extract_candidates(self, question: str) -> Dict[str, Any]:
        # 使用向量检索、LLM等方法提取候选实体
        return {
            "metrics": [...],
            "attributes": [...],
            "enum_values": {...}
        }

# 使用自定义提取器
pipeline = NL2SQLPipeline(use_simple_extractor=False)
pipeline.candidate_extractor = CustomCandidateExtractor()
```

## 项目结构

```
nl2sql_dspy/
├── README.md                    # 本文件
├── entity_map.json5             # 语义层配置
├── src/
│   ├── __init__.py
│   ├── ir_models.py             # IR 数据模型 (Pydantic)
│   ├── ir_parsers.py            # DSPy 解析器模块
│   ├── text_to_ir.py            # Text-to-IR 主流程
│   ├── sql_compiler.py          # SQL 编译器
│   ├── nl2sql_pipeline.py       # 端到端流程
│   └── readme.md                # 架构说明
├── tests/
│   └── test_pipeline.py         # 端到端测试
└── data/
    ├── entity_map_example.json5
    └── intermediate_representation.json5
```

## 配置说明

### IR 结构支持的 SQL 特性

✅ **支持:**
- SELECT 列投影(简单列、聚合、计算字段)
- WHERE 过滤(比较、IN、LIKE、嵌套 AND/OR)
- GROUP BY 分组
- HAVING 聚合过滤
- ORDER BY 排序
- LIMIT / OFFSET 分页
- 自动 JOIN(基于外键关系)

❌ **不支持(需要扩展):**
- 子查询
- 窗口函数
- CTE (WITH 子句)
- UNION / INTERSECT / EXCEPT
- CASE 语句

### 语义层配置

`entity_map.json5` 包含三个主要部分:

1. **entities**: 业务实体到物理列的映射
   - `attribute`: 简单列(可用于过滤、分组)
   - `metric`: 指标(通常需要聚合)

2. **foreign_keys**: 表之间的关系(用于自动生成 JOIN)

3. **enum_values**: 属性的合法取值(帮助 LLM 规范化值)

## 测试

### 运行所有测试

```bash
python tests/test_pipeline.py
```

### 测试覆盖

- ✓ 基本查询(SELECT, WHERE)
- ✓ 聚合查询(COUNT, SUM, AVG, GROUP BY)
- ✓ 复杂查询(多条件过滤、排序、分页)
- ✓ 错误处理(边界情况)

## 候选实体提取

### SimpleCandidateExtractor

项目使用 `SimpleCandidateExtractor` 进行候选实体提取:

**工作原理:**
- 基于关键词模糊匹配从 `entity_map.json5` 中筛选相关实体
- 检查实体名称的子串是否出现在用户问题中
- 如果匹配数量过少,返回所有候选实体

**优点:**
- 简单快速,无需额外依赖
- 适合快速原型和中小规模应用

**扩展方向:**
- 使用向量检索进行语义匹配
- 结合 LLM 理解问题意图
- 添加实体树结构支持层级关系

## 开发指南

### 添加新的操作符

1. 在 IR 定义中添加新的操作符类型(Pydantic 模型)
2. 在 `sql_compiler.py` 的 `_map_operator()` 中添加映射
3. 更新文档和测试

### 优化 LLM 提示

使用 DSPy 的优化器(如 `BootstrapFewShot`):

```python
from dspy.teleprompt import BootstrapFewShot

# 准备训练数据
trainset = [...]

# 优化
optimizer = BootstrapFewShot(metric=your_metric)
optimized_pipeline = optimizer.compile(
    pipeline.ir_generator,
    trainset=trainset
)
```

### 扩展语义层

添加新表时:
1. 在 `entity_map.json5` 中定义所有实体
2. 添加必要的 foreign_keys
3. 更新 enum_values
4. 运行测试验证

## 常见问题

### Q: 为什么使用三阶段 LLM 调用?

A: 分离职责可以提高准确性:
- 阶段1: 解构主要子句(SELECT, GROUP BY等)
- 阶段2: 专注解析 WHERE 条件
- 阶段3: 专注解析 HAVING 条件

### Q: 如何提高小模型的性能?

A: 使用 DSPy 的 few-shot 优化:
```python
from dspy.teleprompt import BootstrapFewShot
optimizer = BootstrapFewShot(metric=accuracy_metric)
optimized = optimizer.compile(module, trainset=examples)
```

### Q: 支持哪些数据库?

A: 当前生成的 SQL 是通用的。要支持特定数据库方言:
- 修改 `sql_compiler.py` 中的 `_build_limit_offset()`
- 添加数据库特定的类型转换

## 贡献

欢迎提交 Issue 和 Pull Request!

## 许可证

MIT License

## 相关资源

- [DSPy 文档](https://dspy-docs.vercel.app/)
- [Pydantic 文档](https://docs.pydantic.dev/)
- [JSON5 规范](https://json5.org/)

---

**作者**: NL2SQL-IR Team  
**最后更新**: 2025-11-04
